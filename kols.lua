...
LPH_JIT_MAX(
    function()
        ...
        local WallCheck = function(destination, ignore)
            if getgenv().kols.Misc.WallCheck then
        ...
        task.spawn(function () while task.wait() do if getgenv().kols.Resolver.AutoResolve == true then checkVelocity(prey or prey2, getgenv().kols.Resolver.Positive, getgenv().kols.Resolver.Negative) end end end)
        ...
        function TargetChecks(Target)
            if getgenv().kols.Misc.UnlockedOnDeath == true and Target.Character then
        ...
        function predictTargets(Target, Value)
            return Target.Character[getgenv().kols.Silent.Part].CFrame +
                (Target.Character[getgenv().kols.Silent.Part].Velocity * Value)
        end
            ...
            function()
                if prey then
                    if prey ~= nil and getgenv().kols.Silent.Enabled and getgenv().kols.Silent.ClosestPart == true then
                        getgenv().kols.Silent["Part"] = tostring(GetClosestBodyPart(prey.Character))
                    end
                end
                if prey2 then
                    if
                     ...
                     then
                        getgenv().kols.Tracer["Part"] = tostring(GetClosestBodyPart(prey2.Character))
                    end
                end
            end
        )
        ...
        local TracerPrediction = function(Target, Value)
            return Target.Character[getgenv().kols.Tracer.Part].Position +
                (Target.Character[getgenv().kols.Tracer.Part].Velocity / Value)
        end
        ...
        RS.RenderStepped:Connect(
            function()
                if
                 ...
                 then
                    local Main = CFrame.new(Camera.CFrame.p, TracerPrediction(prey2, getgenv().kols.Tracer.Pred))
                    Camera.CFrame =
                        Camera.CFrame:Lerp(
                        Main,
                ...
                elseif prey2 ~= nil and getgenv().kols.Tracer.Enabled and getgenv().kols.Tracer.Smoothness == false then
                    Camera.CFrame =
                        CFrame.new(Camera.CFrame.Position, TracerPrediction(prey2, getgenv().kols.Tracer.Pred))
                end
            end
        )
            ...
            function(self, v)
                if Mouse and (table.find(properties, v)) then
                    ...
                    if prey ~= nil and getgenv().kols.Silent.Enabled and not TargetChecks(prey) then
                        local endpoint = predictTargets(prey, getgenv().kols.Silent.Pred)
